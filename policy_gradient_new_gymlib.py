...
class PolicyNet(nn.Module):
    ...
    def save(self, suffix):
        torch.save(self.state_dict(), f'policy_{suffix}.pt')
    ...
    while(True):
        ...
        for _ in range(batch_episodes):
            ...
            for step in count():
               ...
               # env.render(rgb-array) # change to human to visualize realtime video, at the cost of training speed
...
def eval(policy_file_name):
    ...
    with torch.no_grad():
        policy_net = PolicyNet()
        policy_net.load_state_dict(torch.load(policy_file_name))
